(function ($) {
	...
	var Kobe = window.Kobe || {};

	//Slider Kobe
	var Kobe = window.Kobe || {};
    Kobe.slickSlider = function () {
        //Global
        var global_slider = $('.kobe-slider');
        if (global_slider.length) {
            global_slider.each(function () {
                var list = $(this).find('.kobe-slider__list');
                if ($(this).is('.kobe-slider--style-half')) {
                    $(this).find('.kobe-slider__image-list').slick({
                        slidesToShow: 1,
												slidesToScroll: 1,
												arrows: true,
												autoplay: true,
												...
												pauseOnHover: true,
                        fade: true,
                        asNavFor: $(this).find('.kobe-slider__content-list'),
                        responsive: [
                            {
                                breakpoint: 769,
                                settings: {
                    ...
                    $(this).find('.kobe-slider__content-list').slick({
                        slidesToShow: 1,
                        slidesToScroll: 1,
                        asNavFor: $(this).find('.kobe-slider__image-list'),
                        arrows: false,
                        infinite: true,
                        dots: true,
                        appendDots: $(this).find('.kobe-slider__list'),
                        responsive: [
                            {
                                breakpoint: 768,
                                settings: {
                ...
                else if ($(this).is('.kobe-slider--style-1')) {
                    list.slick({
                        slidesToShow: 1,
												slidesToScroll: 1,
												autoplay: true,
    ...
    Kobe.beefSvg = function () {
		var object = $('.svg-kobe-cow-wrapper');
        if (object.length) {
            var content = $(object).contents(),
                beef_part = content.find('path.kobe-clickable'),
                slider = $('.kobe-slider--with-slider .kobe-slider__list');
            //onClick on cow -> slider change
            beef_part.each(function () {
                    $(this).on('click', function () {
                        var slide_pos = $(this).data('slide');
                        $(this).closest('.kobe-beef-svg').find('path.kobe-clickable').removeClass('active');
                        $(this).addClass('active');
						slider.slick('slickGoTo', slide_pos - 1);
						if(slide_pos==3){
                            $('.kobe-popup-wrapper').addClass('open-pop');
                        }
				})
                        
						//mobile pop up
						if($(window).width() < 992){
							if(!$('.kobe-popup-wrapper').length){
								$('body').append($('<div class="kobe-popup-wrapper cow"><div class="kobe-popup"><a href="#" id="close-conbo" class="btn-close"><i class="la la-remove"></i></a></div></div>'));
							}
								else{
								$('.kobe-slider.kobe-slider--with-slider').appendTo('.kobe-popup-wrapper .kobe-popup');
								$(this).on('click', function () {
									$('.kobe-popup-wrapper').addClass('open-pop');
								});
								//Debug iOS
								$(document).on('touchstart', function(event) {
									if (!$(event.target).closest(".kobe-popup-wrapper.open-pop .kobe-popup").length) {
									$(".kobe-popup-wrapper.cow").removeClass("open-pop");
									}
								});
							}
						}
            ...
            //slider change -> highlight on cow change
            $(slider).on('beforeChange', function (event, slick, currentSlide, nextSlide) {
                var activeSlide = beef_part.closest('.kobe-beef-svg').find('[data-slide="' + (nextSlide + 1) + '"]');
                $(activeSlide).closest('.kobe-beef-svg').find('path.kobe-clickable').removeClass('active');
...
// 	});
	...
	$('.toolbar-btn').keydown(function (e) {
		// ESCAPE key pressed
		if (e.keyCode == 27) {
